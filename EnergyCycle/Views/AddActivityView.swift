...
struct AddActivityView: View {
    ...
    @State private var showingRecommendation: Bool
    
    // Инициализатор для предзаполнения из рекомендации
    init(viewModel: EnergyCycleViewModel, activityType: ActivityType? = nil, recommendedTime: Date? = nil) {
        self.viewModel = viewModel
        _selectedType = State(initialValue: activityType ?? .deepWork)
        _scheduledTime = State(initialValue: recommendedTime ?? Date())
        _showingRecommendation = State(initialValue: recommendedTime != nil)
    }
